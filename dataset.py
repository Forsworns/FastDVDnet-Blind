...
class ValDataset(Dataset):
	...
	def __init__(self, valsetdir=None, gtvalsetdir=None, gray_mode=False, num_input_frames=NUMFRXSEQ_VAL):
		...
		gts_dirs = sorted(glob.glob(os.path.join(gtvalsetdir, VALSEQPATT)))
		gts = []
		for gt_dir in gts_dirs:
			gt, _, _ = open_sequence(gt_dir, gray_mode, expand_if_needed=False, \
							 max_num_fr=num_input_frames)
			# gt is [num_frames, C, H, W]
			gts.append(gt)

		self.gts = gts

	def __getitem__(self, index):
		return torch.from_numpy(self.sequences[index]), torch.from_numpy(self.gts[index])
