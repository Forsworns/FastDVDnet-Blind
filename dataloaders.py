...
class VideoReaderPipeline(Pipeline):
	...
	def define_graph(self):
		...
		'''
		noisy = self.reader(name="Reader")
		ground_truth = self.gt_reader(name="GTReader")
		x = self.uniform()
		y = self.uniform()
		cn = self.crop(noisy, crop_pos_x=x, crop_pos_y=y)
		cg = self.crop(ground_truth, crop_pos_x=x,crop_pos_y=y)
		cn = self.transpose(cn)
		cg = self.transpose(cg)
		return cn, cg
