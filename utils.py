...
def normalize_augment(datain, ctrl_fr_idx):
	...
	def transform(sample, gt):
		...
		# transf = choices(aug_list, w_aug) # random augmention
		transf = aug_list # no augmentation

		# transform all images in array
		return transf[0](sample), transf[0](gt)

	img_train, gt_train = datain['noisy'], datain['gt']
	...
	gt_train = gt_train.view(gt_train.size()[0], -1, \
							   gt_train.size()[-2], gt_train.size()[-1]) / 255.
	#augment
	img_train, gt_train = transform(img_train, gt_train)
