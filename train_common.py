...
def validate_and_log(model_temp, dataset_val, valnoisestd, temp_psz, writer, \
	...
	psnr_val = 0
	with torch.no_grad():
		for seq_val, gt_val in dataset_val:
			seq_val = seq_val.cuda()
			out_val = denoise_seq_fastdvdnet(seq=seq_val, \
											temp_psz=temp_psz,\
											model_temporal=model_temp)
			psnr_val += batch_psnr(out_val.cpu(), gt_val.squeeze_(), 1.)
	...
	try:
		...
		if epoch == 0:
			...
			img = tutils.make_grid(gt_val.data[idx].clamp(0., 1.),\
									nrow=2, normalize=False, scale_each=False)
			imgn = tutils.make_grid(seq_val.data[idx].clamp(0., 1.),\
									nrow=2, normalize=False, scale_each=False)
